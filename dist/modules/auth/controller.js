...
export const getSignIn = async (req, res) => {
    ...
    const modifiedLoginHtml = loginHtml.replaceAll("{application_name}", req.clientInfo?.applicationName);
...
export const generateToken = async (req, res) => {
    ...
    const hashedAuthorizationCode = await cryptoUtils.hashContent(authorizationCode);
    ...
    const { token: refreshToken, hashedToken: hashedRefreshToken } = await cryptoUtils.generateHash();
...
export const refreshToken = async (req, res) => {
    ...
    const accessToken = jwt.sign(claims, PRIVATE_KEY, { algorithm: "RS256" });
    const { token: newRefreshToken, hashedToken: hashedNewRefreshToken } = await cryptoUtils.generateHash();
    const refreshTokenExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
...
export const signInUser = async (req, res) => {
    ...
    const isPasswordValid = await bcryptUtils.compareHash(password, user?.password || "");
    if (!isPasswordValid) {
        throw ApiError.unauthorized("Invalid credentials");
    }
    const { token: authorizationCode, hashedToken: hashedAuthorizationCode } = await cryptoUtils.generateHash();
...
export const signUpUser = async (req, res) => {
    const { firstName, lastName, email, gender, dateOfBirth, password, avatarUrl, } = req.body;
